...
const Hero = () => {
  ...
  const activeBanner = banners.find((b) => b.active) || {
    ...
    imageUrl: "/hero-lander.jpeg",
    linkUrl: "/products",
  };

  if (isLoading) {
    return (
      <div className="h-96 w-full animate-pulse bg-stone-200 mt-5 rounded-2xl"></div>
    );
  }
  ...
  return (
    <section className="relative rounded-2xl mt-5 bg-gradient-to-br from-primary-dark via-primary to-primary-light text-white overflow-hidden py-16 md:py-16 px-4 sm:px-6 lg:px-8">
      ...
      <div className="absolute top-1/2 left-1/4 -translate-y-1/2 w-96 h-96 rounded-full bg-secondary/15 blur-3xl"></div>
      <div className="absolute bottom-0 right-10 w-80 h-80 rounded-full bg-accent/10 blur-3xl"></div>
